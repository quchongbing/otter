...
:math:`T_e=1,10,30` eV.  The available reference package contains the paper's
2TTCP HNC+bridge, DFT-MD, and YOCP HNC+bridge curves for panels 2(a), 2(c),
and 2(d).  No numerical data are available here for the 3 eV panel 2(b).
...
cells at 10 and 30 eV :cite:p:`JohnsonEtAl2025`.
...
to load checksummed results, or set it to ``False`` to run all three
...
The reference files record article panels, units, method labels, and SHA-256
checksums, but not whether the coordinates were digitized or author-provided.
The article does not attach an open-data license to these point sets.  They
are therefore distributed with attribution and license status
``NOASSERTION`` and are not covered by Otter's BSD software license.  See the
manifest under
