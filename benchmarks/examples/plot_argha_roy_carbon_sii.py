...
This complete, directly executable benchmark calculates carbon static ion
structure factors with the current Otter average-atom -> pseudoatom ->
QOZ/HNC workflow and compares them with DFT-MD data provided by Dr. Argha Roy
(private communication; unpublished).  Edit the input block below:
...
``USE_PRECOMPUTED_DATA = False``
    ...
    strictly accepted NPZ files and rejection diagnostics under
    ``benchmarks/outputs/argha_roy_carbon_sii/gallery_recomputed``, and include
    the results only when all five states pass the reliability gates.  No
    legacy PAMD curve or external producer script is imported.
...
because that interpretation has not been confirmed.  An intermediate Otter
migration called these curves ``DMF-MD``; the original plotting source and the
maintainer's current identification are ``DFT-MD``, which is the label restored
here.  Redistribution permission is not yet recorded, so these numerical
files are distributed with attribution and license status ``NOASSERTION``.
...
# A maintenance-only environment override makes it possible to regenerate the
# private baseline without editing this tracked file.  Ordinary users can
# simply change USE_PRECOMPUTED_DATA above.
if os.environ.get("OTTER_RECOMPUTE_ARGHA_ROY", "0") == "1":
    USE_PRECOMPUTED_DATA = False
# =============================================================================
